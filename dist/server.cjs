...
async function hashPassword(plainPassword) {
  return import_bcrypt.default.hash(plainPassword, env.bcryptSaltRounds);
}
async function comparePassword(plainPassword, hashedPassword) {
  return import_bcrypt.default.compare(plainPassword, hashedPassword);
}
...
function signToken(payload) {
  ...
  return import_jsonwebtoken.default.sign(payload, env.jwtSecret, options);
}
...
app.use((0, import_cors.default)({ origin: env.corsOrigin }));
